...
interface Reservation {
    ...
    title: string; // 숙소 이름 (Stay title)
    checkInDate: string;
    checkOutDate: string;
    amount: number;
    // 백엔드에서 제공하는 다른 필드가 있다면 여기에 추가할 수 있습니다.
  accountId: string;
  accountName: string;
  email: string;
  phoneNumber: string;
...
const MyPage: React.FC = () => {
    ...
    // 📌 [기존 useEffect 수정] 마이페이지 정보 로드 후 예약 목록 로드 시작
    useEffect(() => {
        if (!token) {
            // 토큰이 없으면 로딩 해제 후 로그인 유도 (JSX에서 처리됨)
            setLoading(false);
            return;
        }

        loadMyPage(); 
        loadReservations(); // 예약 목록도 함께 로드 시작
    }, [token, navigate]);
                ...
                <div className="mypage-right">
                    ...
                    <div className="reservation-card" style={{ 
                        ...
                        {reservations.length === 0 ? (
                            ...
                            <ul style={{ listStyleType: 'none', padding: 0, margin: '10px 0 0' }}>
                                {reservations.map((res) => (
                                    ...
                                    >
                                        <strong style={{ display: 'block', fontSize: '1.1em', color: '#2c3e50' }}>{res.title}</strong>
                                        <p style={{ margin: '5px 0' }}>
                                            기간: {res.checkInDate} ~ {res.checkOutDate}
                                        </p>
                                        <p style={{fontWeight: 'bold' }}>
                                            결제 금액: {res.amount.toLocaleString()}원
                                        </p>
                                    </li>
                                ))}
                            </ul>
