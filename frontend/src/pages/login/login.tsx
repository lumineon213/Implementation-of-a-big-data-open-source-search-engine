...
const [otpQrUri, setOtpQrUri] = useState<string | null>(null);
  ...
  const handleLogin = async (e?: React.MouseEvent | React.KeyboardEvent) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8484/api/login/login",
        loginData
      );
...
if (res.data.needOtpSetup) {
  ...
  const qrRes = await axios.get(
    "http://localhost:8484/api/login/otp/setup",
    {
      headers: {
        Authorization: res.data.otpToken
      }
    }
  );

  if (qrRes.data.success) {
    setOtpQrUri(qrRes.data.otpUri);
  } else {
    alert("OTP QR 생성 실패");
  }
...
{needOtpSetup && otpQrUri && (
  <div className="otp-container">
    <div className="otp-title">관리자 OTP 등록</div>
    ...
    <p className="otp-guide">
      Google Authenticator 앱으로 QR을 스캔하세요
    </p>
  </div>
)}
      ...
      const msg = error.response?.data?.msg || (err as any)?.message || "";

      // 존재하지 않는 아이디 처리(원래 니가 원하던 로직)
      ...
      if (newAttempts >= 5) {
        ...
        alert(msg || `로그인 실패 (${newAttempts}/5)`);
      }
  ...
  const verifyOtp = async () => {
    if (!otpCode || otpCode.length !== 6) {
      alert("6자리 OTP를 입력하세요");
      return;
    }

    if (!otpToken) {
      alert("OTP 토큰이 없습니다. 다시 로그인해주세요.");
      return;
    }

    try {
      const res = await axios.post(
        "http://localhost:8484/api/login/otp/verify",
        { otpToken, otpCode }
      );

      if (!res.data.success) {
        alert(res.data.msg || "OTP 인증 실패");
        return;
      }

      if (!res.data.token) {
        alert("OTP 인증은 성공했는데 토큰이 없습니다. 서버 응답 확인 필요");
        return;
      }
              ...
              <div className="form-section">
                <div className="input-field-wrapper">
                  <input
                    ...
                    onChange={(e) =>
                      setLoginData({ ...loginData, accountId: e.target.value })
                    }
                ...
                <div className="input-field-wrapper">
                  <input
                    ...
                    className="input-field"
                    onChange={(e) =>
                      setLoginData({ ...loginData, accountPw: e.target.value })
                    }
                    onKeyPress={handleKeyPress}
                  />
                </div>
                ...
                {/*  OTP 최초 등록 QR UI (백엔드가 qrUri 내려줄 때) */}
                {needOtpSetup && (
...
/>
                        ...
                        <p style={{ marginTop: 10, fontSize: 13, color: "#555" }}>
                          Google Authenticator(또는 Authy)로 QR을 스캔한 뒤,
                          생성된 6자리 코드를 입력하세요.
                        </p>
                      ...
                      <p style={{ fontSize: 13, color: "#c62828" }}>
                        QR 데이터(otpQrUri)가 서버에서 안 내려왔습니다.
                        백엔드 응답에 qrUri를 넣어줘야 QR이 뜹니다.
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="signup-form">
                ...
                <div>
                  <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    <button className="verify-btn" onClick={checkAccountId}>
                      중복확인
                    </button>
                  </div>
                  {idChecked && (
                ...
                <div>
                  <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    <button className="verify-btn" onClick={sendEmailCode}>
                      인증번호
                    </button>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "6px" }}>
                    ...
                    />
                    <button className="verify-btn" onClick={verifyEmailCodeCheck}>
                      확인
                    </button>
                  </div>

                  {emailVerified && (
                    <p className="verify-success">✔ 이메일 인증 완료</p>
                  )}
                </div>
                  ...
                  <div style={{ position: "relative" }}>
                    <input
                      ...
                      onChange={(e) =>
                        setSignupData({
                          ...signupData,
                          accountPwConfirm: e.target.value
                        })
                      }
                      style={{
                        paddingRight: "45px",
                        borderColor: passwordNotMatch
                          ? "#dc3545"
                          : passwordMatch
                          ? "#28a745"
                          : undefined
        ...
        <div className="modal-overlay" onClick={() => setShowTermsModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="terms-section">
                ...
                <div className="terms-content">
                  ...
                  <p>
                    본 약관은 우리 부산 GO? 서비스의 이용조건 및 절차에 관한 사항을
                    규정함을 목적으로 합니다.
                  </p>
                  <br />
                  <p>
              ...
              <div className="terms-section">
                ...
                <div className="terms-content">
                  ...
                  <p>
                    본 약관은 우리 부산 GO?가 제공하는 위치기반서비스에 대해 회사와
                    이용자의 권리·의무 및 책임사항, 기타 필요한 사항을 규정함을
                    목적으로 합니다.
                  </p>
                  <br />
                  <p>
                    <strong>제2조 (위치정보의 수집 및 이용)</strong>
                  </p>
            ...
            <div className="modal-footer">
              <button className="modal-confirm-btn" onClick={handleTermsConfirm}>
                동의하고 회원가입하기
              </button>
            </div>
          </div>
